...
게시글 상세 제목 바에 나열된 작업 버튼을 제거하고, 사용자가 선택한 게시글 또는 댓글에 맞는 컨텍스트 팝업 메뉴로 제공한다. 긴 버튼 목록으로 본문 너비가 줄어드는 문제를 해결하고 현재 작업 대상이 게시글인지 댓글인지 명확하게 표시한다.

### 7.2 메뉴 열기와 닫기

- 게시글 또는 댓글을 한 번 클릭하면 해당 콘텐츠를 선택한다.
- 게시글 또는 댓글 영역에서 마우스 오른쪽 버튼을 누르면 해당 콘텐츠를 선택하고 컨텍스트 메뉴를 연다.
- 팝업 메뉴는 제목 바의 `⋮` 버튼으로 열지 않는다.
- 제목 바의 `⋮` 메뉴 버튼은 제거한다.
- 텍스트 선택, 링크, 버튼, 입력 요소, 첨부파일을 클릭할 때는 메뉴를 열지 않는다.
- 메뉴 밖 클릭, `Esc`, 메뉴 항목 실행, 대상 변경 시 메뉴를 닫는다.
- 선택된 콘텐츠에는 옅은 배경과 테두리를 표시한다.
- 제목 바에는 선택 대상 문구와 작업 메뉴를 표시하지 않는다.
- 제목 바 오른쪽에는 상세 화면 닫기 버튼만 둔다.
- 현재 대상이 게시글인지 댓글인지는 선택된 콘텐츠 카드의 배경과 테두리, 그리고 팝업 메뉴의 접근성 label로 구분한다.
...
- 수정과 삭제는 게시글 권한이 있을 때만 표시한다.
- 핀 고정/해제는 게시글에만 표시하고 관리자 또는 허용된 작성자 권한을 적용한다.
- 다른 채널로 이동은 이동 권한이 있을 때만 표시한다.
- 삭제는 빨간색, 이동은 주황색으로 구분한다.
...
- 댓글에는 핀 고정/해제를 표시하지 않는다.
- 수정과 삭제는 선택한 댓글의 권한을 기준으로 표시한다.
- 다른 채널 이동·복사는 새 게시글 또는 대상 게시글의 댓글 방식을 후속 모달에서 선택한다.
- AgenticAI와 DM에는 게시글 전체가 아니라 선택한 댓글과 댓글 deep link를 전달한다.

### 7.5 메뉴 실행

- 수정: 메뉴를 닫고 선택 콘텐츠의 편집 화면을 연다.
- 삭제: 메뉴를 닫고 확인 대화상자를 연다.
- 복사와 링크 복사: 실행 후 메뉴를 닫고 완료 상태를 표시한다.
- 핀 고정/해제: 실행 후 현재 게시글 상태와 메뉴 label을 갱신한다.
- 다른 채널 이동·복사: 메뉴를 먼저 닫고 채널 전송 모달을 연다.
- AgenticAI: 메뉴를 닫고 AgenticAI panel을 연다.
- DM: 메뉴를 닫고 대화 선택 모달을 연다.

### 7.6 접근성과 위치

- 컨텍스트 메뉴는 마우스 오른쪽 클릭 위치를 기준으로 표시한다.
- 팝업은 `role="menu"`, 항목은 `role="menuitem"`을 사용한다.
- 위·아래 방향키로 항목을 이동하고 `Enter` 또는 `Space`로 실행한다.
- `Esc`로 닫은 뒤 메뉴를 연 버튼에 focus를 돌려준다.
- 팝업은 클릭한 좌표에 배치하고 viewport 오른쪽이나 아래 경계를 넘으면 안쪽으로 위치를 보정한다.
- 모바일에서도 누르기 쉬운 최소 40px 높이의 메뉴 항목을 사용한다.

### 7.7 구현 대상

| 파일 | 변경 |
|---|---|
| `src/components/chat/PostDetailPane.jsx` | 제목 바 버튼을 컨텍스트 메뉴로 교체하고 게시글/댓글 선택에 따라 메뉴 구성 분리 |
| `MDfiles/UI.md` | 컨텍스트 메뉴 동작 및 권한·접근성 기준 기록 |

`server/rag_train.py`는 UI 메뉴 변경과 관련이 없으므로 수정하지 않는다.

...
export default function SearchResultsArea({ onSelectResult }) {
  ...
  async function handleItemClick(item) {
    // Find the team and channel objects
    ...
    if (team && channel) {
      ...
      // item.type can be 'post' or 'comment'
      // If it's a comment, we want to open the parent post
    ...
    <div className="flex-1 flex flex-col bg-[#1e1c30] min-w-0">
      ...
      <div className="h-14 px-6 border-b border-white/10 flex items-center justify-between flex-shrink-0 bg-[#1e1c30]/50 backdrop-blur-md sticky top-0 z-10">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-white font-bold text-lg">검색 결과</h2>
            <p className="text-white/40 text-xs">"{searchTerm}" 키워드에 대한 결과 {searchResults.length}건</p>
          </div>
        </div>
      </div>
        ...
        <div className="max-w-4xl mx-auto space-y-4">
          {isSearching ? (
            <div className="flex flex-col items-center justify-center py-20 gap-4">
              ...
              <p className="text-white/40">검색 결과를 불러오는 중...</p>
            </div>
            ...
            <div className="text-center py-20">
              ...
              <p className="text-white/40 text-lg">"{searchTerm}"에 대한 검색 결과가 없습니다.</p>
              <button 
                onClick={closeSearch}
                  ...
                  <div className="min-w-0">
                    ...
                    {item.type === 'comment' && item.postContent && (
                      <div className="mb-2 p-3 bg-black/20 rounded-xl border border-white/5">
                        <p className="text-white/30 text-[11px] uppercase font-bold mb-1">원문 게시글</p>
